...
api/advisor.py  Advisor digital · función serverless y núcleo compartido
dev_server.py   servidor local que reutiliza ese mismo núcleo
...
```bash
python3 dev_server.py     # http://127.0.0.1:8765
```
...
Vercel: el directorio de salida es `web/` y la función Python vive en `api/`.
La clave se configura como variable de entorno del proyecto (**nunca** en el repositorio).
